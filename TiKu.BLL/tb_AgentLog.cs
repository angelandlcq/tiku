...
{
    //经销商日志
    public partial class tb_AgentLog
    {
        #region 成员
        /// <summary>
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_AgentLog";
        #endregion

        /// <summary>
        /// 增加一条数据
        /// </summary>
        public bool Add(TiKu.Model.tb_AgentLog model)
        {
            return EntityQuery<TiKu.Model.tb_AgentLog>.Instance.Add(model);
        }

        /// <summary>
        ///修改
        /// </summary>
        public bool Update(TiKu.Model.tb_AgentLog model)
        {
            return EntityQuery<TiKu.Model.tb_AgentLog>.Instance.Update(model);
        }

        /// <summary>
        ///删除
        /// </summary>
        public int Delete(TiKu.Model.tb_AgentLog model)
        {
            return EntityQuery<TiKu.Model.tb_AgentLog>.Instance.Delete(model);
        }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_AgentLog GetModel(int ID)
        {
            TiKu.Model.tb_AgentLog model = new TiKu.Model.tb_AgentLog();
            model.ID = ID;
            EntityQuery<TiKu.Model.tb_AgentLog>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_AgentLog> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_AgentLog> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_AgentLog>.Instance.GetList(top,
                                                                               "ID,Action,AgentID,Level,Msg,AddDate",
                                                                               "tb_AgentLog",
                                                                               where,
                                                                               orders,
                                                                               parameters);
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_AgentLog> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,Action,AgentID,Level,Msg,AddDate FROM [{0}] WHERE 1=1 ", TableName);
            ...
            List<TiKu.Model.tb_AgentLog> list = new List<TiKu.Model.tb_AgentLog>();

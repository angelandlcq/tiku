...
{
	 	//套装组合
		public partial class tb_SuitCompages
	{
	    #region 成员
        /// <summary>
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_SuitCompages";
        #endregion
   		     
	    /// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(TiKu.Model.tb_SuitCompages model)
		{
		  return EntityQuery<TiKu.Model.tb_SuitCompages>.Instance.Add(model);
		}
   		     
	    /// <summary>
		///修改
		/// </summary>
	   public bool Update(TiKu.Model.tb_SuitCompages model)
	   {
	    return EntityQuery<TiKu.Model.tb_SuitCompages>.Instance.Update(model);
	   }
	   
	   	 /// <summary>
		///删除
		/// </summary>
	   public int Delete(TiKu.Model.tb_SuitCompages model)
	   {
	    return EntityQuery<TiKu.Model.tb_SuitCompages>.Instance.Delete(model);
	   }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_SuitCompages GetModel(int ID)
        {
            TiKu.Model.tb_SuitCompages model = new TiKu.Model.tb_SuitCompages();
            model.ID=ID;
            EntityQuery<TiKu.Model.tb_SuitCompages>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_SuitCompages> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_SuitCompages> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_SuitCompages>.Instance.GetList(top,
                                                                               "ID,CourseID,LimitedChapter,CreatedOn,CreatedBy,State",
                                                                               "tb_SuitCompages",
                                                                               where,
                                                                               orders,
                                                                               parameters);
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_SuitCompages> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,CourseID,LimitedChapter,CreatedOn,CreatedBy,State FROM [{0}] WHERE 1=1 ",TableName);
            ...
            List<TiKu.Model.tb_SuitCompages> list = new List<TiKu.Model.tb_SuitCompages>();

...
{
	 	//经销商消费记录
		public partial class tb_CostRecord
	{
	    #region 成员
        /// <summary>
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_CostRecord";
        #endregion
   		     
	    /// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(TiKu.Model.tb_CostRecord model)
		{
		  return EntityQuery<TiKu.Model.tb_CostRecord>.Instance.Add(model);
		}
   		     
	    /// <summary>
		///修改
		/// </summary>
	   public bool Update(TiKu.Model.tb_CostRecord model)
	   {
	    return EntityQuery<TiKu.Model.tb_CostRecord>.Instance.Update(model);
	   }
	   
	   	 /// <summary>
		///删除
		/// </summary>
	   public int Delete(TiKu.Model.tb_CostRecord model)
	   {
	    return EntityQuery<TiKu.Model.tb_CostRecord>.Instance.Delete(model);
	   }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_CostRecord GetModel(int ID)
        {
            TiKu.Model.tb_CostRecord model = new TiKu.Model.tb_CostRecord();
            model.ID=ID;
            EntityQuery<TiKu.Model.tb_CostRecord>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_CostRecord> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_CostRecord> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_CostRecord>.Instance.GetList(top,
                                                                               "ID,Amount,AgentID,Note,AddDate",
                                                                               "tb_CostRecord",
                                                                               where,
                                                                               orders,
                                                                               parameters);
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_CostRecord> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,Amount,AgentID,Note,AddDate FROM [{0}] WHERE 1=1 ",TableName);
            ...
            List<TiKu.Model.tb_CostRecord> list = new List<TiKu.Model.tb_CostRecord>();

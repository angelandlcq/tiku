...
namespace TiKu.BLL
{
		...
		public partial class tb_Collection
	{
        ...
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_Collection";
        #endregion
   		     
	    /// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(TiKu.Model.tb_Collection model)
		{
		  return EntityQuery<TiKu.Model.tb_Collection>.Instance.Add(model);
		}
   		     
	    /// <summary>
		///修改
		/// </summary>
	   public bool Update(TiKu.Model.tb_Collection model)
	   {
	    return EntityQuery<TiKu.Model.tb_Collection>.Instance.Update(model);
	   }
	   
	   	 /// <summary>
		///删除
		/// </summary>
	   public int Delete(TiKu.Model.tb_Collection model)
	   {
	    return EntityQuery<TiKu.Model.tb_Collection>.Instance.Delete(model);
	   }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_Collection GetModel(decimal ID)
        {
            TiKu.Model.tb_Collection model = new TiKu.Model.tb_Collection();
            model.ID=ID;
            EntityQuery<TiKu.Model.tb_Collection>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_Collection> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_Collection> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_Collection>.Instance.GetList(top,
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_Collection> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,UserID,QuestionID FROM [{0}] WHERE 1=1 ",TableName);
            ...
            List<TiKu.Model.tb_Collection> list = new List<TiKu.Model.tb_Collection>();

...
{
	 	//后台管理页面
		public partial class tb_AdminPage
	{
	    #region 成员
        /// <summary>
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_AdminPage";
        #endregion
   		     
	    /// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(TiKu.Model.tb_AdminPage model)
		{
		  return EntityQuery<TiKu.Model.tb_AdminPage>.Instance.Add(model);
		}
   		     
	    /// <summary>
		///修改
		/// </summary>
	   public bool Update(TiKu.Model.tb_AdminPage model)
	   {
	    return EntityQuery<TiKu.Model.tb_AdminPage>.Instance.Update(model);
	   }
	   
	   	 /// <summary>
		///删除
		/// </summary>
	   public int Delete(TiKu.Model.tb_AdminPage model)
	   {
	    return EntityQuery<TiKu.Model.tb_AdminPage>.Instance.Delete(model);
	   }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_AdminPage GetModel(int ID)
        {
            TiKu.Model.tb_AdminPage model = new TiKu.Model.tb_AdminPage();
            model.ID=ID;
            EntityQuery<TiKu.Model.tb_AdminPage>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_AdminPage> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_AdminPage> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_AdminPage>.Instance.GetList(top,
                                                                               "ID,Names,LinkUrl,Target,ImageUrl,Orders,Del",
                                                                               "tb_AdminPage",
                                                                               where,
                                                                               orders,
                                                                               parameters);
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_AdminPage> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,Names,LinkUrl,Target,ImageUrl,Orders,Del FROM [{0}] WHERE 1=1 ",TableName);
            ...
            List<TiKu.Model.tb_AdminPage> list = new List<TiKu.Model.tb_AdminPage>();

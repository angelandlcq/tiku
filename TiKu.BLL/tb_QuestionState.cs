...
{
	 	//做题记录表0
		public partial class tb_QuestionState
	{
	    #region 成员
        /// <summary>
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_QuestionState";
        #endregion
   		     
	    /// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(TiKu.Model.tb_QuestionState model)
		{
		  return EntityQuery<TiKu.Model.tb_QuestionState>.Instance.Add(model);
		}
   		     
	    /// <summary>
		///修改
		/// </summary>
	   public bool Update(TiKu.Model.tb_QuestionState model)
	   {
	    return EntityQuery<TiKu.Model.tb_QuestionState>.Instance.Update(model);
	   }
	   
	   	 /// <summary>
		///删除
		/// </summary>
	   public int Delete(TiKu.Model.tb_QuestionState model)
	   {
	    return EntityQuery<TiKu.Model.tb_QuestionState>.Instance.Delete(model);
	   }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_QuestionState GetModel(int ID)
        {
            TiKu.Model.tb_QuestionState model = new TiKu.Model.tb_QuestionState();
            model.ID=ID;
            EntityQuery<TiKu.Model.tb_QuestionState>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_QuestionState> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_QuestionState> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_QuestionState>.Instance.GetList(top,
                                                                               "ID,ChapterID,QuestionID,Answer,UserID,AddTime",
                                                                               "tb_QuestionState",
                                                                               where,
                                                                               orders,
                                                                               parameters);
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_QuestionState> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,ChapterID,QuestionID,Answer,UserID,AddTime FROM [{0}] WHERE 1=1 ",TableName);
            ...
            List<TiKu.Model.tb_QuestionState> list = new List<TiKu.Model.tb_QuestionState>();

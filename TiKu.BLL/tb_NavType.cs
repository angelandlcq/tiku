...
{
	 	//管理员权限
		public partial class tb_NavType
	{
	    #region 成员
        /// <summary>
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_NavType";
        #endregion
   		     
	    /// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(TiKu.Model.tb_NavType model)
		{
		  return EntityQuery<TiKu.Model.tb_NavType>.Instance.Add(model);
		}
   		     
	    /// <summary>
		///修改
		/// </summary>
	   public bool Update(TiKu.Model.tb_NavType model)
	   {
	    return EntityQuery<TiKu.Model.tb_NavType>.Instance.Update(model);
	   }
	   
	   	 /// <summary>
		///删除
		/// </summary>
	   public int Delete(TiKu.Model.tb_NavType model)
	   {
	    return EntityQuery<TiKu.Model.tb_NavType>.Instance.Delete(model);
	   }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_NavType GetModel(int ID)
        {
            TiKu.Model.tb_NavType model = new TiKu.Model.tb_NavType();
            model.ID=ID;
            EntityQuery<TiKu.Model.tb_NavType>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_NavType> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_NavType> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_NavType>.Instance.GetList(top,
                                                                               "ID,Names,ParentID,Orders,Enable,Del",
                                                                               "tb_NavType",
                                                                               where,
                                                                               orders,
                                                                               parameters);
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_NavType> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,Names,ParentID,Orders,Enable,Del FROM [{0}] WHERE 1=1 ",TableName);
            ...
            List<TiKu.Model.tb_NavType> list = new List<TiKu.Model.tb_NavType>();

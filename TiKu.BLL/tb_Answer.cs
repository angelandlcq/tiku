...
namespace TiKu.BLL
{
		...
		public partial class tb_Answer
	{
        ...
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_Answer";
        #endregion
   		     
	    /// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(TiKu.Model.tb_Answer model)
		{
		  return EntityQuery<TiKu.Model.tb_Answer>.Instance.Add(model);
		}
   		     
	    /// <summary>
		///修改
		/// </summary>
	   public bool Update(TiKu.Model.tb_Answer model)
	   {
	    return EntityQuery<TiKu.Model.tb_Answer>.Instance.Update(model);
	   }
	   
	   	 /// <summary>
		///删除
		/// </summary>
	   public int Delete(TiKu.Model.tb_Answer model)
	   {
	    return EntityQuery<TiKu.Model.tb_Answer>.Instance.Delete(model);
	   }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_Answer GetModel(int ID)
        {
            TiKu.Model.tb_Answer model = new TiKu.Model.tb_Answer();
            model.ID=ID;
            EntityQuery<TiKu.Model.tb_Answer>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_Answer> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_Answer> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_Answer>.Instance.GetList(top,
                                                                               "ID,Code,QuestionID,State,Answer,Orders,CreatedBy,CreatedOn,ModifyBy,ModifyOn,Del",
                                                                               "tb_Answer",
                                                                               where,
                                                                               orders,
                                                                               parameters);
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_Answer> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,Code,QuestionID,State,Answer,Orders,CreatedBy,CreatedOn,ModifyBy,ModifyOn,Del FROM [{0}] WHERE 1=1 ",TableName);
            ...
            List<TiKu.Model.tb_Answer> list = new List<TiKu.Model.tb_Answer>();

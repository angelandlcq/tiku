...
namespace TiKu.BLL
{
    ...
    public partial class tb_User
    {
        ...
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_User";
        #endregion

        /// <summary>
        /// 增加一条数据
        /// </summary>
        public bool Add(TiKu.Model.tb_User model)
        {
            return EntityQuery<TiKu.Model.tb_User>.Instance.Add(model);
        }

        /// <summary>
        ///修改
        /// </summary>
        public bool Update(TiKu.Model.tb_User model)
        {
            return EntityQuery<TiKu.Model.tb_User>.Instance.Update(model);
        }

        /// <summary>
        ///删除
        /// </summary>
        public int Delete(TiKu.Model.tb_User model)
        {
            return EntityQuery<TiKu.Model.tb_User>.Instance.Delete(model);
        }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_User GetModel(long ID)
        {
            TiKu.Model.tb_User model = new TiKu.Model.tb_User();
            model.ID = ID;
            EntityQuery<TiKu.Model.tb_User>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_User> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_User> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_User>.Instance.GetList(top,
                                                                               "ID,UserName,NickName,UserPwd,PwdSalt,Mobile,Email,State,Del,RegisterTime,RegisterIP",
                                                                               "tb_User",
                                                                               where,
                                                                               orders,
                                                                               parameters);
        ...
        {
            if (null == state || state == DBNull.Value)
            {
                return "未验证";
            }
            ...
            switch (iState)
            {
                case TiKu.Common.Constants.USER.STATE.VERIFYING:
                    return "未验证";
                case TiKu.Common.Constants.USER.STATE.POST:
                    return "已验证/激活";
                case TiKu.Common.Constants.USER.STATE.LOCK:
                    return "已冻结";
            }
            return "--";
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_User> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,UserName,NickName,UserPwd,PwdSalt,Mobile,Email,State,Del,RegisterTime,RegisterIP FROM [{0}] WHERE 1=1 ", TableName);
            ...
            List<TiKu.Model.tb_User> list = new List<TiKu.Model.tb_User>();

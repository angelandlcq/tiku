...
{
	 	//充值记录
		public partial class tb_UserRecharge
	{
	    #region 成员
        /// <summary>
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_UserRecharge";
        #endregion
   		     
	    /// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(TiKu.Model.tb_UserRecharge model)
		{
		  return EntityQuery<TiKu.Model.tb_UserRecharge>.Instance.Add(model);
		}
   		     
	    /// <summary>
		///修改
		/// </summary>
	   public bool Update(TiKu.Model.tb_UserRecharge model)
	   {
	    return EntityQuery<TiKu.Model.tb_UserRecharge>.Instance.Update(model);
	   }
	   
	   	 /// <summary>
		///删除
		/// </summary>
	   public int Delete(TiKu.Model.tb_UserRecharge model)
	   {
	    return EntityQuery<TiKu.Model.tb_UserRecharge>.Instance.Delete(model);
	   }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_UserRecharge GetModel(int ID,long UserID)
        {
            TiKu.Model.tb_UserRecharge model = new TiKu.Model.tb_UserRecharge();
            model.ID=ID;
model.UserID=UserID;
            EntityQuery<TiKu.Model.tb_UserRecharge>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_UserRecharge> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_UserRecharge> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_UserRecharge>.Instance.GetList(top,
                                                                               "ID,UserID,Amount,Balance,AddTime",
                                                                               "tb_UserRecharge",
                                                                               where,
                                                                               orders,
                                                                               parameters);
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_UserRecharge> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,UserID,Amount,Balance,AddTime FROM [{0}] WHERE 1=1 ",TableName);
            ...
            List<TiKu.Model.tb_UserRecharge> list = new List<TiKu.Model.tb_UserRecharge>();

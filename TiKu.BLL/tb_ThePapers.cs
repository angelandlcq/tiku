...
{
	 	//做题试卷
		public partial class tb_ThePapers
	{
	    #region 成员
        /// <summary>
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_ThePapers";
        #endregion
   		     
	    /// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(TiKu.Model.tb_ThePapers model)
		{
		  return EntityQuery<TiKu.Model.tb_ThePapers>.Instance.Add(model);
		}
   		     
	    /// <summary>
		///修改
		/// </summary>
	   public bool Update(TiKu.Model.tb_ThePapers model)
	   {
	    return EntityQuery<TiKu.Model.tb_ThePapers>.Instance.Update(model);
	   }
	   
	   	 /// <summary>
		///删除
		/// </summary>
	   public int Delete(TiKu.Model.tb_ThePapers model)
	   {
	    return EntityQuery<TiKu.Model.tb_ThePapers>.Instance.Delete(model);
	   }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_ThePapers GetModel(int ID)
        {
            TiKu.Model.tb_ThePapers model = new TiKu.Model.tb_ThePapers();
            model.ID=ID;
            EntityQuery<TiKu.Model.tb_ThePapers>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_ThePapers> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_ThePapers> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_ThePapers>.Instance.GetList(top,
                                                                               "ID,UserID,CapterID,AnswerQuantity,Names,CreatedOn,TestTime,Score",
                                                                               "tb_ThePapers",
                                                                               where,
                                                                               orders,
                                                                               parameters);
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_ThePapers> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,UserID,CapterID,AnswerQuantity,Names,CreatedOn,TestTime,Score FROM [{0}] WHERE 1=1 ",TableName);
            ...
            List<TiKu.Model.tb_ThePapers> list = new List<TiKu.Model.tb_ThePapers>();

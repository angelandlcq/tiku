...
namespace TiKu.BLL
{
		...
		public partial class tb_Order
	{
        ...
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_Order";
        #endregion
   		     
	    /// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(TiKu.Model.tb_Order model)
		{
		  return EntityQuery<TiKu.Model.tb_Order>.Instance.Add(model);
		}
   		     
	    /// <summary>
		///修改
		/// </summary>
	   public bool Update(TiKu.Model.tb_Order model)
	   {
	    return EntityQuery<TiKu.Model.tb_Order>.Instance.Update(model);
	   }
	   
	   	 /// <summary>
		///删除
		/// </summary>
	   public int Delete(TiKu.Model.tb_Order model)
	   {
	    return EntityQuery<TiKu.Model.tb_Order>.Instance.Delete(model);
	   }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_Order GetModel(int ID)
        {
            TiKu.Model.tb_Order model = new TiKu.Model.tb_Order();
            model.ID=ID;
            EntityQuery<TiKu.Model.tb_Order>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_Order> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_Order> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_Order>.Instance.GetList(top,
                                                                               "ID,OrderNO,Amount,TenderType,AddTime,State,UserID,Remark",
                                                                               "tb_Order",
                                                                               where,
                                                                               orders,
                                                                               parameters);
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_Order> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,OrderNO,Amount,TenderType,AddTime,State,UserID,Remark FROM [{0}] WHERE 1=1 ",TableName);
            ...
            List<TiKu.Model.tb_Order> list = new List<TiKu.Model.tb_Order>();

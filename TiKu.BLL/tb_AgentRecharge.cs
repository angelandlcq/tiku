...
{
    //经销商充值记录
    public partial class tb_AgentRecharge
    {
        #region 成员
        /// <summary>
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_AgentRecharge";
        #endregion

        /// <summary>
        /// 增加一条数据
        /// </summary>
        public bool Add(TiKu.Model.tb_AgentRecharge model)
        {
            return EntityQuery<TiKu.Model.tb_AgentRecharge>.Instance.Add(model);
        }

        /// <summary>
        ///修改
        /// </summary>
        public bool Update(TiKu.Model.tb_AgentRecharge model)
        {
            return EntityQuery<TiKu.Model.tb_AgentRecharge>.Instance.Update(model);
        }

        /// <summary>
        ///删除
        /// </summary>
        public int Delete(TiKu.Model.tb_AgentRecharge model)
        {
            return EntityQuery<TiKu.Model.tb_AgentRecharge>.Instance.Delete(model);
        }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_AgentRecharge GetModel(int ID)
        {
            TiKu.Model.tb_AgentRecharge model = new TiKu.Model.tb_AgentRecharge();
            model.ID = ID;
            EntityQuery<TiKu.Model.tb_AgentRecharge>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_AgentRecharge> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_AgentRecharge> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_AgentRecharge>.Instance.GetList(top,
                                                                               "ID,AgentID,Amount,Balance,note,AddTime",
                                                                               "tb_AgentRecharge",
                                                                               where,
                                                                               orders,
                                                                               parameters);
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_AgentRecharge> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,AgentID,Amount,Balance,note,AddTime FROM [{0}] WHERE 1=1 ", TableName);
            ...
            List<TiKu.Model.tb_AgentRecharge> list = new List<TiKu.Model.tb_AgentRecharge>();

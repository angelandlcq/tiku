...
{
	 	//已做试题
		public partial class tb_DoneQuestion
	{
	    #region 成员
        /// <summary>
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_DoneQuestion";
        #endregion
   		     
	    /// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(TiKu.Model.tb_DoneQuestion model)
		{
		  return EntityQuery<TiKu.Model.tb_DoneQuestion>.Instance.Add(model);
		}
   		     
	    /// <summary>
		///修改
		/// </summary>
	   public bool Update(TiKu.Model.tb_DoneQuestion model)
	   {
	    return EntityQuery<TiKu.Model.tb_DoneQuestion>.Instance.Update(model);
	   }
	   
	   	 /// <summary>
		///删除
		/// </summary>
	   public int Delete(TiKu.Model.tb_DoneQuestion model)
	   {
	    return EntityQuery<TiKu.Model.tb_DoneQuestion>.Instance.Delete(model);
	   }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_DoneQuestion GetModel(int ID)
        {
            TiKu.Model.tb_DoneQuestion model = new TiKu.Model.tb_DoneQuestion();
            model.ID=ID;
            EntityQuery<TiKu.Model.tb_DoneQuestion>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_DoneQuestion> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_DoneQuestion> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_DoneQuestion>.Instance.GetList(top,
                                                                               "ID,TestID,UserID,Answer,AddDate",
                                                                               "tb_DoneQuestion",
                                                                               where,
                                                                               orders,
                                                                               parameters);
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_DoneQuestion> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,TestID,UserID,Answer,AddDate FROM [{0}] WHERE 1=1 ",TableName);
            ...
            List<TiKu.Model.tb_DoneQuestion> list = new List<TiKu.Model.tb_DoneQuestion>();

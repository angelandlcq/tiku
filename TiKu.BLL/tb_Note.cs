...
namespace TiKu.BLL
{
		...
		public partial class tb_Note
	{
        ...
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_Note";
        #endregion
   		     
	    /// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(TiKu.Model.tb_Note model)
		{
		  return EntityQuery<TiKu.Model.tb_Note>.Instance.Add(model);
		}
   		     
	    /// <summary>
		///修改
		/// </summary>
	   public bool Update(TiKu.Model.tb_Note model)
	   {
	    return EntityQuery<TiKu.Model.tb_Note>.Instance.Update(model);
	   }
	   
	   	 /// <summary>
		///删除
		/// </summary>
	   public int Delete(TiKu.Model.tb_Note model)
	   {
	    return EntityQuery<TiKu.Model.tb_Note>.Instance.Delete(model);
	   }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_Note GetModel(int ID)
        {
            TiKu.Model.tb_Note model = new TiKu.Model.tb_Note();
            model.ID=ID;
            EntityQuery<TiKu.Model.tb_Note>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_Note> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_Note> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_Note>.Instance.GetList(top,
                                                                               "ID,UserID,QuestionID,Note,CreatedOn,IsOpen",
                                                                               "tb_Note",
                                                                               where,
                                                                               orders,
                                                                               parameters);
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_Note> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,UserID,QuestionID,Note,CreatedOn,IsOpen FROM [{0}] WHERE 1=1 ",TableName);
            ...
            List<TiKu.Model.tb_Note> list = new List<TiKu.Model.tb_Note>();

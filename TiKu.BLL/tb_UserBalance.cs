...
{
	 	//消费记录
		public partial class tb_UserBalance
	{
	    #region 成员
        /// <summary>
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_UserBalance";
        #endregion
   		     
	    /// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(TiKu.Model.tb_UserBalance model)
		{
		  return EntityQuery<TiKu.Model.tb_UserBalance>.Instance.Add(model);
		}
   		     
	    /// <summary>
		///修改
		/// </summary>
	   public bool Update(TiKu.Model.tb_UserBalance model)
	   {
	    return EntityQuery<TiKu.Model.tb_UserBalance>.Instance.Update(model);
	   }
	   
	   	 /// <summary>
		///删除
		/// </summary>
	   public int Delete(TiKu.Model.tb_UserBalance model)
	   {
	    return EntityQuery<TiKu.Model.tb_UserBalance>.Instance.Delete(model);
	   }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_UserBalance GetModel(int ID)
        {
            TiKu.Model.tb_UserBalance model = new TiKu.Model.tb_UserBalance();
            model.ID=ID;
            EntityQuery<TiKu.Model.tb_UserBalance>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_UserBalance> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_UserBalance> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_UserBalance>.Instance.GetList(top,
                                                                               "ID,UserID,Amount,Note,Balance,AddTime",
                                                                               "tb_UserBalance",
                                                                               where,
                                                                               orders,
                                                                               parameters);
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_UserBalance> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,UserID,Amount,Note,Balance,AddTime FROM [{0}] WHERE 1=1 ",TableName);
            ...
            List<TiKu.Model.tb_UserBalance> list = new List<TiKu.Model.tb_UserBalance>();

...
namespace TiKu.BLL
{
    ...
    public partial class tb_Test
    {
        ...
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_Test";
        #endregion

        /// <summary>
        /// 增加一条数据
        /// </summary>
        public bool Add(TiKu.Model.tb_Test model)
        {
            return EntityQuery<TiKu.Model.tb_Test>.Instance.Add(model);
        }

        /// <summary>
        ///修改
        /// </summary>
        public bool Update(TiKu.Model.tb_Test model)
        {
            return EntityQuery<TiKu.Model.tb_Test>.Instance.Update(model);
        }

        /// <summary>
        ///删除
        /// </summary>
        public int Delete(TiKu.Model.tb_Test model)
        {
            return EntityQuery<TiKu.Model.tb_Test>.Instance.Delete(model);
        }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_Test GetModel(int ID)
        {
            TiKu.Model.tb_Test model = new TiKu.Model.tb_Test();
            model.ID = ID;
            EntityQuery<TiKu.Model.tb_Test>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_Test> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_Test> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_Test>.Instance.GetList(top,
                                                                               "ID,Names",
                                                                               "tb_Test",
                                                                               where,
                                                                               orders,
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_Test> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,Names FROM [{0}] WHERE 1=1 ", TableName);
            ...
            List<TiKu.Model.tb_Test> list = new List<TiKu.Model.tb_Test>();
        ...
        public int UpdateDataSet(string cmdText, DataSet ds, Action<DataSet> action)
        {
            return TiKu.DataDriver.AdoHelper.UpdateDataSet(cmdText, ds, action);
        }

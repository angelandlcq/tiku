...
/*=================================================================================================
 * 
 * className:阅读简答信息表(BLL)
 * author:1058736170@qq.com
 * date:2015-04-02 18:58
 * modifyBy:1058736170@qq.com
 * modifyOn:2015-04-02 18:58
...
namespace TiKu.BLL
{
		...
		public partial class tb_Material
	{
        ...
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_Material";
        #endregion
   		     
	    /// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(TiKu.Model.tb_Material model)
		{
		  return EntityQuery<TiKu.Model.tb_Material>.Instance.Add(model);
		}
   		     
	    /// <summary>
		///修改
		/// </summary>
	   public bool Update(TiKu.Model.tb_Material model)
	   {
	    return EntityQuery<TiKu.Model.tb_Material>.Instance.Update(model);
	   }
	   
	   	 /// <summary>
		///删除
		/// </summary>
	   public int Delete(TiKu.Model.tb_Material model)
	   {
	    return EntityQuery<TiKu.Model.tb_Material>.Instance.Delete(model);
	   }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_Material GetModel(int ID)
        {
            TiKu.Model.tb_Material model = new TiKu.Model.tb_Material();
            model.ID=ID;
            EntityQuery<TiKu.Model.tb_Material>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_Material> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_Material> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_Material>.Instance.GetList(top,
                                                                               "ID,Context,CreatedOn,CreatedBy,ModifyOn,ModifyBy",
                                                                               "tb_Material",
                                                                               where,
                                                                               orders,
                                                                               parameters);
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_Material> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,Context,CreatedOn,CreatedBy,ModifyOn,ModifyBy FROM [{0}] WHERE 1=1 ",TableName);
            ...
            List<TiKu.Model.tb_Material> list = new List<TiKu.Model.tb_Material>();

...
{
	 	//错题
		public partial class tb_ErrorQuestion
	{
	    #region 成员
        /// <summary>
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_ErrorQuestion";
        #endregion
   		     
	    /// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(TiKu.Model.tb_ErrorQuestion model)
		{
		  return EntityQuery<TiKu.Model.tb_ErrorQuestion>.Instance.Add(model);
		}
   		     
	    /// <summary>
		///修改
		/// </summary>
	   public bool Update(TiKu.Model.tb_ErrorQuestion model)
	   {
	    return EntityQuery<TiKu.Model.tb_ErrorQuestion>.Instance.Update(model);
	   }
	   
	   	 /// <summary>
		///删除
		/// </summary>
	   public int Delete(TiKu.Model.tb_ErrorQuestion model)
	   {
	    return EntityQuery<TiKu.Model.tb_ErrorQuestion>.Instance.Delete(model);
	   }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_ErrorQuestion GetModel(int ID)
        {
            TiKu.Model.tb_ErrorQuestion model = new TiKu.Model.tb_ErrorQuestion();
            model.ID=ID;
            EntityQuery<TiKu.Model.tb_ErrorQuestion>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_ErrorQuestion> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_ErrorQuestion> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_ErrorQuestion>.Instance.GetList(top,
                                                                               "ID,UserID,QuestionID,ChapterID,ErrorNum,CreatedOn",
                                                                               "tb_ErrorQuestion",
                                                                               where,
                                                                               orders,
                                                                               parameters);
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_ErrorQuestion> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,UserID,QuestionID,ChapterID,ErrorNum,CreatedOn FROM [{0}] WHERE 1=1 ",TableName);
            ...
            List<TiKu.Model.tb_ErrorQuestion> list = new List<TiKu.Model.tb_ErrorQuestion>();

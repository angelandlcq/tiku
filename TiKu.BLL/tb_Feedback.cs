...
namespace TiKu.BLL
{
		...
		public partial class tb_Feedback
	{
        ...
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_Feedback";
        #endregion
   		     
	    /// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(TiKu.Model.tb_Feedback model)
		{
		  return EntityQuery<TiKu.Model.tb_Feedback>.Instance.Add(model);
		}
   		     
	    /// <summary>
		///修改
		/// </summary>
	   public bool Update(TiKu.Model.tb_Feedback model)
	   {
	    return EntityQuery<TiKu.Model.tb_Feedback>.Instance.Update(model);
	   }
	   
	   	 /// <summary>
		///删除
		/// </summary>
	   public int Delete(TiKu.Model.tb_Feedback model)
	   {
	    return EntityQuery<TiKu.Model.tb_Feedback>.Instance.Delete(model);
	   }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_Feedback GetModel(int ID)
        {
            TiKu.Model.tb_Feedback model = new TiKu.Model.tb_Feedback();
            model.ID=ID;
            EntityQuery<TiKu.Model.tb_Feedback>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_Feedback> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_Feedback> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_Feedback>.Instance.GetList(top,
                                                                               "ID,UserID,Contents,AddDate,State,Audit",
                                                                               "tb_Feedback",
                                                                               where,
                                                                               orders,
                                                                               parameters);
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_Feedback> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,UserID,Contents,AddDate,State,Audit FROM [{0}] WHERE 1=1 ",TableName);
            ...
            List<TiKu.Model.tb_Feedback> list = new List<TiKu.Model.tb_Feedback>();

...
{
	 	//章节考点关系表
		public partial class tb_ChapterQuestion
	{
	    #region 成员
        /// <summary>
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_ChapterQuestion";
        #endregion
   		     
	    /// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(TiKu.Model.tb_ChapterQuestion model)
		{
		  return EntityQuery<TiKu.Model.tb_ChapterQuestion>.Instance.Add(model);
		}
   		     
	    /// <summary>
		///修改
		/// </summary>
	   public bool Update(TiKu.Model.tb_ChapterQuestion model)
	   {
	    return EntityQuery<TiKu.Model.tb_ChapterQuestion>.Instance.Update(model);
	   }
	   
	   	 /// <summary>
		///删除
		/// </summary>
	   public int Delete(TiKu.Model.tb_ChapterQuestion model)
	   {
	    return EntityQuery<TiKu.Model.tb_ChapterQuestion>.Instance.Delete(model);
	   }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_ChapterQuestion GetModel(int ID)
        {
            TiKu.Model.tb_ChapterQuestion model = new TiKu.Model.tb_ChapterQuestion();
            model.ID=ID;
            EntityQuery<TiKu.Model.tb_ChapterQuestion>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_ChapterQuestion> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_ChapterQuestion> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_ChapterQuestion>.Instance.GetList(top,
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_ChapterQuestion> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,ChapterID,QuestionID FROM [{0}] WHERE 1=1 ",TableName);
            ...
            List<TiKu.Model.tb_ChapterQuestion> list = new List<TiKu.Model.tb_ChapterQuestion>();

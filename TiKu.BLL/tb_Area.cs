...
namespace TiKu.BLL
{
		...
		public partial class tb_Area
	{
        ...
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_Area";
        #endregion
   		     
	    /// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(TiKu.Model.tb_Area model)
		{
		  return EntityQuery<TiKu.Model.tb_Area>.Instance.Add(model);
		}
   		     
	    /// <summary>
		///修改
		/// </summary>
	   public bool Update(TiKu.Model.tb_Area model)
	   {
	    return EntityQuery<TiKu.Model.tb_Area>.Instance.Update(model);
	   }
	   
	   	 /// <summary>
		///删除
		/// </summary>
	   public int Delete(TiKu.Model.tb_Area model)
	   {
	    return EntityQuery<TiKu.Model.tb_Area>.Instance.Delete(model);
	   }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_Area GetModel(int ID)
        {
            TiKu.Model.tb_Area model = new TiKu.Model.tb_Area();
            model.ID=ID;
            EntityQuery<TiKu.Model.tb_Area>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_Area> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_Area> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_Area>.Instance.GetList(top,
                                                                               "ID,AreaCode,AreaName,Orders,State",
                                                                               "tb_Area",
                                                                               where,
                                                                               orders,
                                                                               parameters);
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_Area> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,AreaCode,AreaName,Orders,State FROM [{0}] WHERE 1=1 ",TableName);
            ...
            List<TiKu.Model.tb_Area> list = new List<TiKu.Model.tb_Area>();

...
{
	 	//难度
		public partial class tb_Difficulty
	{
	    #region 成员
        /// <summary>
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_Difficulty";
        #endregion
   		     
	    /// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(TiKu.Model.tb_Difficulty model)
		{
		  return EntityQuery<TiKu.Model.tb_Difficulty>.Instance.Add(model);
		}
   		     
	    /// <summary>
		///修改
		/// </summary>
	   public bool Update(TiKu.Model.tb_Difficulty model)
	   {
	    return EntityQuery<TiKu.Model.tb_Difficulty>.Instance.Update(model);
	   }
	   
	   	 /// <summary>
		///删除
		/// </summary>
	   public int Delete(TiKu.Model.tb_Difficulty model)
	   {
	    return EntityQuery<TiKu.Model.tb_Difficulty>.Instance.Delete(model);
	   }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_Difficulty GetModel(int ID)
        {
            TiKu.Model.tb_Difficulty model = new TiKu.Model.tb_Difficulty();
            model.ID=ID;
            EntityQuery<TiKu.Model.tb_Difficulty>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_Difficulty> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_Difficulty> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_Difficulty>.Instance.GetList(top,
                                                                               "ID,QuestionID,Rate,UserID,CreatedOn",
                                                                               "tb_Difficulty",
                                                                               where,
                                                                               orders,
                                                                               parameters);
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_Difficulty> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,QuestionID,Rate,UserID,CreatedOn FROM [{0}] WHERE 1=1 ",TableName);
            ...
            List<TiKu.Model.tb_Difficulty> list = new List<TiKu.Model.tb_Difficulty>();

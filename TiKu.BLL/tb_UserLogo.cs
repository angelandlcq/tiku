...
{
	 	//会员日志
		public partial class tb_UserLogo
	{
	    #region 成员
        /// <summary>
        /// 表名
        /// </summary>
        private readonly string TableName = "tb_UserLogo";
        #endregion
   		     
	    /// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(TiKu.Model.tb_UserLogo model)
		{
		  return EntityQuery<TiKu.Model.tb_UserLogo>.Instance.Add(model);
		}
   		     
	    /// <summary>
		///修改
		/// </summary>
	   public bool Update(TiKu.Model.tb_UserLogo model)
	   {
	    return EntityQuery<TiKu.Model.tb_UserLogo>.Instance.Update(model);
	   }
	   
	   	 /// <summary>
		///删除
		/// </summary>
	   public int Delete(TiKu.Model.tb_UserLogo model)
	   {
	    return EntityQuery<TiKu.Model.tb_UserLogo>.Instance.Delete(model);
	   }
        ...
        /// <returns></returns>
        public TiKu.Model.tb_UserLogo GetModel(int ID)
        {
            TiKu.Model.tb_UserLogo model = new TiKu.Model.tb_UserLogo();
            model.ID=ID;
            EntityQuery<TiKu.Model.tb_UserLogo>.Instance.Fill(model);
            return model;
        }
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_UserLogo> GetList(int top,
            string where,
            string orders,
            IEnumerable parameters)
        {
            List<TiKu.Model.tb_UserLogo> list = TiKu.DataDriver.Entity.EntityQuery<TiKu.Model.tb_UserLogo>.Instance.GetList(top,
                                                                               "ID,UserID,ActionTyp,TableName,Level,Msg,Event,AddTime",
                                                                               "tb_UserLogo",
                                                                               where,
                                                                               orders,
                                                                               parameters);
        ...
        /// <returns></returns>
        public List<TiKu.Model.tb_UserLogo> GetListPager(int startIndex,
                                                    int endIndex,
                                                    string where,
                                                    string orders,
                                                    IEnumerable parameters,
                                                    out int iRowCount)
        {
            ...
            stbCmdText.AppendFormat("SELECT Row_Number() over(order by ID desc) as Row,ID,UserID,ActionTyp,TableName,Level,Msg,Event,AddTime FROM [{0}] WHERE 1=1 ",TableName);
            ...
            List<TiKu.Model.tb_UserLogo> list = new List<TiKu.Model.tb_UserLogo>();

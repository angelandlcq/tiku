...
/*******************************************************弹出消息*******************************************************************/
(function (target) {
    target.ShowMsg = function (title, content, url, type, callBack) {
        var dialog = $.dialog({
            title: title,
            content: content,
            min: false,
    ...
    target.loading = function () {
        return $.dialog({ id: "pageloading", title: false, lock: true });
    }
    //关闭loading加载
    target.closeLoading = function (_id) {
        var _loadId = _id || "pageloading";
        if ($.dialog.list[_loadId]) {
            $.dialog.list[_loadId].close();
        }
    }
})(window);